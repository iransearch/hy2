...
ANYTLS_INSTANCES_DIR="$ANYTLS_DIR/instances"
ANYTLS_BINARY="/usr/bin/AT"
...
gecko_warp_apply_xray_json() {
  ...
  if gecko_warp_is_enabled; then
    ...
    jq --arg mode "$(gecko_warp_mode)" --argjson port "$port" --argjson domains "$domains" '
      ...
      | if $mode == "all" then
          .routing.rules = ([{type:"field", network:"tcp,udp", outboundTag:"warp"}] + .routing.rules)
...
enable_gecko_real_outbound_via_warp() {
  local detected_port proxy_port backup_dir mode="${1:-selective}"
  [[ "$mode" == selective || "$mode" == all ]] || return 1
  clear
  echo "======================================================="
  echo " Enable unified WARP outbound: $mode"
  echo "======================================================="
  echo "Applies to client traffic in Hysteria2, Reality, XHTTP and AnyTLS."
  echo "Host system routing (including SSH) is not changed."
  [ "$(id -u)" -eq 0 ] || { echo "Please run as root."; return 1; }
  if [[ "$mode" == all ]]; then
    echo "All client TCP/UDP traffic will use WARP; no DIRECT fallback."
    echo "UDP requires UDP support in the local WARP SOCKS5 proxy."
  else
    echo "Listed sites use WARP; other traffic uses DIRECT."
    show_default_gecko_warp_routes || return 1
  fi
  echo
  ...
  rm -rf "$backup_dir"
  echo "Unified WARP enabled successfully: $mode."
...
show_gecko_warp_status() {
  ...
  if [[ -n "$(anytls_ids)" ]]; then
    local anytls_id
    while read -r anytls_id; do
      printf 'AnyTLS %-16s: ' "$anytls_id"
      if jq -e 'any(.outbounds[]?; (.tag // "") == "warp")' "$(anytls_dir "$anytls_id")/config.json" >/dev/null; then
        echo 'warp: enabled'
      else
        echo 'warp: disabled'
      fi
...
gecko_warp_proxy_menu() {
  ...
  echo "One route list: Hysteria2 + Reality + XHTTP + AnyTLS."
    echo
    ...
    case "$WARP_CHOICE" in
      ...
      7) enable_gecko_real_outbound_via_warp all; read -rp "Press Enter to return to WARP menu..." ;;
      0) return ;;
      *) echo "Invalid choice."; sleep 1 ;;
    esac
  done
...
XBOARD_ISP_SERVICE="xboard-isp-socks-bridge.service"

xboard_isp_ok()   { echo -e "\e[32m[OK]\e[0m $1"; }
xboard_isp_err()  { echo -e "\e[31m[ERR]\e[0m $1"; }
xboard_isp_info() { echo -e "\e[34m[*]\e[0m $1"; }
xboard_isp_warn() { echo -e "\e[33m[!]\e[0m $1"; }

xboard_isp_require_root() {
  [ "$(id -u)" -eq 0 ] || { xboard_isp_err "Please run as root."; return 1; }
}

xboard_isp_shell_quote() {
  python3 -c 'import sys,shlex; print(shlex.quote(sys.argv[1]))' "$1"
}

xboard_isp_parse_upstream_uri() {
  python3 - "$1" <<'PY_PARSE_ISP_SOCKS'
import sys, urllib.parse, shlex
raw = sys.argv[1].strip()
if not raw:
    raise SystemExit('Empty SOCKS proxy value.')
if '://' not in raw:
    raw = 'socks5://' + raw
u = urllib.parse.urlsplit(raw)
if not u.hostname or not u.port:
    raise SystemExit('Invalid format. Use: user:pass@host:port')
user = urllib.parse.unquote(u.username or '')
password = urllib.parse.unquote(u.password or '')
if not user or not password:
    raise SystemExit('Username and password are required. Use: user:pass@host:port')
if u.port < 1 or u.port > 65535:
    raise SystemExit('Invalid port.')
print('XBOARD_ISP_UPSTREAM_HOST=' + shlex.quote(u.hostname))
print('XBOARD_ISP_UPSTREAM_PORT=' + shlex.quote(str(u.port)))
print('XBOARD_ISP_UPSTREAM_USER=' + shlex.quote(user))
print('XBOARD_ISP_UPSTREAM_PASS=' + shlex.quote(password))
PY_PARSE_ISP_SOCKS
}

xboard_isp_write_bridge_script() {
  cat > "$XBOARD_ISP_SCRIPT" <<'PY_BRIDGE_SCRIPT'
#!/usr/bin/env python3
import os
import socket
import struct
...
if not UPSTREAM_HOST or not UPSTREAM_PORT:
    print('UPSTREAM_HOST/UPSTREAM_PORT are required', file=sys.stderr)
    sys.exit(1)
...
def read_socks_addr(sock):
    ...
    if atyp == 1:
        ...
        name = recv_exact(sock, ln)
        addr = bytes([ln]) + name
        host = name.decode('utf-8', 'replace')
    elif atyp == 4:
    ...
    return atyp, addr, port_bytes, host, port

def send_client_reply(client, rep):
    # SOCKS5 reply: ver, rep, rsv, atyp, bind addr, bind port
    client.sendall(b'\x05' + bytes([rep]) + b'\x00\x01\x00\x00\x00\x00\x00\x00')

def upstream_connect(atyp, addr, port_bytes):
    up = socket.create_connection((UPSTREAM_HOST, UPSTREAM_PORT), timeout=CONNECT_TIMEOUT)
    up.settimeout(CONNECT_TIMEOUT)

    if UPSTREAM_USER or UPSTREAM_PASS:
        up.sendall(b'\x05\x01\x02')
        resp = recv_exact(up, 2)
        if resp != b'\x05\x02':
            raise OSError('upstream did not accept username/password auth')
        ub = UPSTREAM_USER.encode()
        pb = UPSTREAM_PASS.encode()
        if len(ub) > 255 or len(pb) > 255:
            raise OSError('upstream username/password too long')
        up.sendall(b'\x01' + bytes([len(ub)]) + ub + bytes([len(pb)]) + pb)
        auth_resp = recv_exact(up, 2)
        if auth_resp != b'\x01\x00':
            raise OSError('upstream authentication failed')
    else:
        up.sendall(b'\x05\x01\x00')
        resp = recv_exact(up, 2)
        if resp != b'\x05\x00':
            raise OSError('upstream did not accept no-auth')

    up.sendall(b'\x05\x01\x00' + bytes([atyp]) + addr + port_bytes)
    head = recv_exact(up, 4)
    if head[0] != 5:
        raise OSError('invalid upstream SOCKS version')
    if head[1] != 0:
        raise OSError(f'upstream connect failed, reply={head[1]}')
    bind_atyp = head[3]
    if bind_atyp == 1:
        recv_exact(up, 4)
    elif bind_atyp == 3:
        ...
        raise OSError('invalid upstream bind address type')
    recv_exact(up, 2)
    up.settimeout(None)
    return up
...
def handle_client(client, peer):
    ...
    try:
        ...
        req = recv_exact(client, 3)
        if req[0] != 5 or req[1] != 1:
            send_client_reply(client, 7)  # command not supported
            return
        atyp, addr, port_bytes, host, port = read_socks_addr(client)
        try:
            upstream = upstream_connect(atyp, addr, port_bytes)
        except Exception as e:
            log(f'CONNECT {host}:{port} failed: {e}')
            send_client_reply(client, 5)
            return
        send_client_reply(client, 0)
        log(f'CONNECT {host}:{port} via {UPSTREAM_HOST}:{UPSTREAM_PORT}')
        client.settimeout(None)
        relay(client, upstream)
    except Exception as e:
        log(f'client {peer} error: {e}')
    finally:
...
log(f'listening on {LISTEN_HOST}:{LISTEN_PORT}, upstream {UPSTREAM_HOST}:{UPSTREAM_PORT}')
...
try:
    while not stop_event.is_set():
        try:
            ...
            break
        t = threading.Thread(target=handle_client, args=(client, peer), daemon=True)
        t.start()
finally:
    try:
        server.close()
    ...
    log('stopped')
PY_BRIDGE_SCRIPT
  chmod +x "$XBOARD_ISP_SCRIPT"
}

xboard_isp_write_service() {
  cat > "/etc/systemd/system/$XBOARD_ISP_SERVICE" <<EOF_XBOARD_ISP_SERVICE
[Unit]
Description=Xboard ISP Dedicated Proxies Local SOCKS Bridge
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
EnvironmentFile=$XBOARD_ISP_ENV
ExecStart=/usr/bin/python3 $XBOARD_ISP_SCRIPT
Restart=always
RestartSec=3
LimitNOFILE=1048576
...
[Install]
WantedBy=multi-user.target
EOF_XBOARD_ISP_SERVICE
}

xboard_isp_load_env() {
  [ -f "$XBOARD_ISP_ENV" ] && . "$XBOARD_ISP_ENV" 2>/dev/null || true
}

xboard_isp_install_bridge() {
  clear
  echo "======================================================="
  echo " Install Local SOCKS Bridge for Xboard ISP Proxies"
  echo "======================================================="
  echo "Xboard panel outbound should connect to:"
  echo "  $XBOARD_ISP_LOCAL_ADDR:$XBOARD_ISP_LOCAL_PORT"
  echo "This node will authenticate to your remote ISP SOCKS proxy."
  echo "======================================================="
  xboard_isp_require_root || return 1
  command -v python3 >/dev/null 2>&1 || { xboard_isp_err "python3 is required."; return 1; }

  echo
  read -rsp "Remote ISP SOCKS (user:pass@host:port): " ISP_SOCKS_URI
  echo
  if ! xboard_isp_parse_upstream_uri "$ISP_SOCKS_URI" >/tmp/xboard_isp_upstream.env 2>/tmp/xboard_isp_parse.err; then
    xboard_isp_err "Invalid SOCKS value."
    cat /tmp/xboard_isp_parse.err 2>/dev/null || true
    rm -f /tmp/xboard_isp_upstream.env /tmp/xboard_isp_parse.err
    return 1
  fi
  . /tmp/xboard_isp_upstream.env
  rm -f /tmp/xboard_isp_upstream.env /tmp/xboard_isp_parse.err

  echo
  echo "Local listen : $XBOARD_ISP_LOCAL_ADDR:$XBOARD_ISP_LOCAL_PORT"
  echo "Remote SOCKS : $XBOARD_ISP_UPSTREAM_HOST:$XBOARD_ISP_UPSTREAM_PORT"
  echo "Remote auth  : enabled"
  echo
  read -rp "Install/update local bridge now? [Y/n]: " confirm
  confirm="${confirm:-Y}"
  case "$confirm" in n|N|no|NO|No) echo "Cancelled."; return 0 ;; esac

  mkdir -p "$XBOARD_ISP_DIR"
  chmod 700 "$XBOARD_ISP_DIR" 2>/dev/null || true
  {
    echo "LISTEN_HOST=$(xboard_isp_shell_quote "$XBOARD_ISP_LOCAL_ADDR")"
    echo "LISTEN_PORT=$(xboard_isp_shell_quote "$XBOARD_ISP_LOCAL_PORT")"
    echo "UPSTREAM_HOST=$(xboard_isp_shell_quote "$XBOARD_ISP_UPSTREAM_HOST")"
    echo "UPSTREAM_PORT=$(xboard_isp_shell_quote "$XBOARD_ISP_UPSTREAM_PORT")"
    echo "UPSTREAM_USER=$(xboard_isp_shell_quote "$XBOARD_ISP_UPSTREAM_USER")"
    echo "UPSTREAM_PASS=$(xboard_isp_shell_quote "$XBOARD_ISP_UPSTREAM_PASS")"
    echo "CONNECT_TIMEOUT=15"
    echo "IDLE_TIMEOUT=300"
  } > "$XBOARD_ISP_ENV"
  chmod 600 "$XBOARD_ISP_ENV" 2>/dev/null || true

  xboard_isp_write_bridge_script
  xboard_isp_write_service
  systemctl daemon-reload
  systemctl enable --now "$XBOARD_ISP_SERVICE"
  systemctl restart "$XBOARD_ISP_SERVICE"
  sleep 1

  if systemctl is-active --quiet "$XBOARD_ISP_SERVICE"; then
    xboard_isp_ok "Local SOCKS bridge is running on $XBOARD_ISP_LOCAL_ADDR:$XBOARD_ISP_LOCAL_PORT"
  else
    xboard_isp_err "Bridge service failed to start. Recent logs:"
    journalctl -u "$XBOARD_ISP_SERVICE" -n 30 --no-pager
    return 1
  fi

  echo
  echo "Use this outbound in Xboard panel:"
  xboard_isp_show_panel_json_raw
  echo
  xboard_isp_test_local_bridge || true
}

xboard_isp_show_panel_json_raw() {
  cat <<EOF_XBOARD_ISP_JSON
[
  {
    "tag": "$XBOARD_ISP_TAG",
    "protocol": "socks",
    "settings": {
      "servers": [
        {
          "address": "$XBOARD_ISP_LOCAL_ADDR",
          "port": $XBOARD_ISP_LOCAL_PORT
        }
      ]
    }
  }
...
xboard_isp_show_panel_json() {
  clear
  echo "======================================================="
  echo " Xboard Panel Outbound JSON"
  echo "======================================================="
  echo "Put this in Xboard panel custom outbounds."
  echo "Routing rules in panel must use outboundTag: $XBOARD_ISP_TAG"
  echo "======================================================="
  xboard_isp_show_panel_json_raw
}

xboard_isp_test_local_bridge() {
  echo
  echo "Testing local SOCKS bridge: $XBOARD_ISP_LOCAL_ADDR:$XBOARD_ISP_LOCAL_PORT"
  if ! command -v curl >/dev/null 2>&1; then
    xboard_isp_warn "curl is not installed. Skipping test."
    return 0
  fi
  if curl --socks5-hostname "$XBOARD_ISP_LOCAL_ADDR:$XBOARD_ISP_LOCAL_PORT" --max-time 20 -fsSL https://www.cloudflare.com/cdn-cgi/trace 2>/tmp/xboard_isp_local_curl.err | grep -E '^(ip|colo|warp)='; then
    xboard_isp_ok "Local bridge test succeeded."
    rm -f /tmp/xboard_isp_local_curl.err
    return 0
  else
    xboard_isp_err "Local bridge test failed."
    [ -s /tmp/xboard_isp_local_curl.err ] && cat /tmp/xboard_isp_local_curl.err
    rm -f /tmp/xboard_isp_local_curl.err
    return 1
  fi
}

xboard_isp_test_menu() {
  clear
  echo "======================================================="
  echo " Test Xboard Local ISP SOCKS Bridge"
  echo "======================================================="
  systemctl is-active --quiet "$XBOARD_ISP_SERVICE" || xboard_isp_warn "$XBOARD_ISP_SERVICE is not active."
  xboard_isp_test_local_bridge
}
...
xboard_isp_status() {
  clear
  echo "======================================================="
  echo " Xboard ISP Dedicated Proxies Local Bridge Status"
  echo "======================================================="
  echo "Local SOCKS : $XBOARD_ISP_LOCAL_ADDR:$XBOARD_ISP_LOCAL_PORT"
  echo "Panel tag   : $XBOARD_ISP_TAG"
  echo "Service     : $XBOARD_ISP_SERVICE"
  echo
  if [ -f "$XBOARD_ISP_ENV" ]; then
    xboard_isp_load_env
    echo "Remote SOCKS: ${UPSTREAM_HOST:-unknown}:${UPSTREAM_PORT:-unknown}"
    echo "Remote auth : $([ -n "${UPSTREAM_USER:-}" ] && echo enabled || echo disabled)"
  else
    echo "Remote SOCKS: not configured"
  fi
  echo
  systemctl status "$XBOARD_ISP_SERVICE" --no-pager 2>/dev/null | head -25 || echo "$XBOARD_ISP_SERVICE is not installed."
  echo
  echo "Listening port:"
  ss -lntp 2>/dev/null | grep -E "127\.0\.0\.1:$XBOARD_ISP_LOCAL_PORT\b|:$XBOARD_ISP_LOCAL_PORT\b" || echo "No listener found on $XBOARD_ISP_LOCAL_ADDR:$XBOARD_ISP_LOCAL_PORT"
  echo
  echo "Last logs:"
  journalctl -u "$XBOARD_ISP_SERVICE" -n 20 --no-pager 2>/dev/null || true
}

xboard_isp_restart() {
  clear
  echo "Restarting $XBOARD_ISP_SERVICE ..."
  xboard_isp_require_root || return 1
  systemctl restart "$XBOARD_ISP_SERVICE" || return 1
  sleep 1
  systemctl status "$XBOARD_ISP_SERVICE" --no-pager | head -20 || true
}

xboard_isp_uninstall() {
  clear
  echo "======================================================="
  echo " Uninstall Xboard Local ISP SOCKS Bridge"
  echo "======================================================="
  echo "This removes only the local bridge service and its stored upstream credentials."
  echo "It does NOT change Xboard panel settings."
  echo "======================================================="
  xboard_isp_require_root || return 1
  read -rp "Remove local bridge now? [y/N]: " confirm
  case "$confirm" in y|Y|yes|YES|Yes) ;; *) echo "Cancelled."; return 0 ;; esac
  systemctl disable --now "$XBOARD_ISP_SERVICE" >/dev/null 2>&1 || true
  rm -f "/etc/systemd/system/$XBOARD_ISP_SERVICE"
  rm -f "$XBOARD_ISP_SCRIPT"
  rm -rf "$XBOARD_ISP_DIR"
  systemctl daemon-reload
  xboard_isp_ok "Local bridge removed."
}
...
xboard_socks_routing_menu() {
  while true; do
    ...
    echo "Local SOCKS       : $XBOARD_ISP_LOCAL_ADDR:$XBOARD_ISP_LOCAL_PORT"
    echo
    echo " 1) Install / update local ISP SOCKS bridge"
    echo " 2) Show Xboard panel outbound JSON"
    echo " 3) Test local SOCKS bridge"
    echo " 4) Show bridge status / logs"
    echo " 5) Restart bridge"
    echo " 6) Uninstall bridge"
    echo " 0) Back"
    echo "======================================================="
    read -rp "Choose: " XBOARD_ISP_CHOICE
    case "$XBOARD_ISP_CHOICE" in
      1) xboard_isp_install_bridge; read -rp "Press Enter to return to menu..." ;;
      2) xboard_isp_show_panel_json; read -rp "Press Enter to return to menu..." ;;
      3) xboard_isp_test_menu; read -rp "Press Enter to return to menu..." ;;
      4) xboard_isp_status; read -rp "Press Enter to return to menu..." ;;
      5) xboard_isp_restart; read -rp "Press Enter to return to menu..." ;;
      6) xboard_isp_uninstall; read -rp "Press Enter to return to menu..." ;;
      0) return ;;
      *) echo "Invalid choice."; sleep 1 ;;
    esac
  done
...
# =======================================================
# AnyTLS multi-config manager (sing-box-extended)
# =======================================================
anytls_dir() { printf '%s/%s' "$ANYTLS_INSTANCES_DIR" "$1"; }
anytls_unit() { printf 'anytls-%s.service' "$1"; }
anytls_valid_id() { [[ "$1" =~ ^[A-Za-z0-9][A-Za-z0-9_-]{0,31}$ ]]; }
anytls_valid_name() { [[ "$1" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,31}$ ]]; }
anytls_valid_duration() { [[ "$1" =~ ^[1-9][0-9]*(ms|s|m|h)$ ]]; }
anytls_valid_password() {
  [[ -n "$1" && ${#1} -le 256 ]] || return 1
  ! LC_ALL=C grep -q '[[:cntrl:]]' <<<"$1"
}
anytls_alpn_json() {
  jq -Rn --arg v "$1" '$v|split(",")|map(gsub("^[[:space:]]+|[[:space:]]+$";""))|map(select(length>0))'
}
anytls_ids() {
  local f
  for f in "$ANYTLS_INSTANCES_DIR"/*/meta.json; do [[ -f "$f" ]] && basename "$(dirname "$f")"; done 2>/dev/null | sort
}
anytls_select() {
  local id meta picked
  local -a rows=()
  while read -r id; do
    [[ -n "$id" ]] || continue
    meta="$(anytls_dir "$id")/meta.json"
    rows+=("$id" "$(jq -r '(.security|ascii_upcase)+" | :"+(.port|tostring)+" | "+.sni' "$meta")")
  done < <(anytls_ids)
  ((${#rows[@]})) || { whiptail --msgbox "No AnyTLS configs exist." 9 48; return 1; }
  picked=$(whiptail --title "AnyTLS configs" --menu "Select a config:" 22 84 12 "${rows[@]}" 2>&1 >/dev/tty) || return 1
  printf '%s' "$picked"
}
anytls_port_busy() {
  ...
  if ss -H -ltn 2>/dev/null | awk '{print $4}' | grep -Eq "(^|:)$port$"; then
    ...
    tui_error "TCP port $port is already listening."; return 0
  fi
  return 1
}
anytls_padding_json() {
  local mode="$1" value="$2" json line
  [[ "$mode" == default ]] && { printf '[]'; return; }
  json="$(jq -Rn --arg v "$value" '$v|split(";")|map(gsub("^[[:space:]]+|[[:space:]]+$";""))|map(select(length>0))')" || return 1
...
anytls_collect() {
  ...
  local default_security=tls default_cert=letsencrypt default_insecure=false default_pm=default default_pt=""
  local default_check=30s default_timeout=30s default_min=0 default_cm=""
  local default_alpn="" default_min_tls=1.2 default_max_tls=1.3 default_handshake=15s
  if [[ -f "$current" ]]; then
    old="$(jq -r .id "$current")"; default_id="$old"; default_port="$(jq -r .port "$current")"
    default_address="$(jq -r .address "$current")"; default_sni="$(jq -r .sni "$current")"
    default_security="$(jq -r .security "$current")"; default_cert="$(jq -r '.certificate_mode//"letsencrypt"' "$current")"
    default_insecure="$(jq -r '.insecure//false' "$current")"; default_pm="$(jq -r '.padding_mode//"default"' "$current")"
    default_pt="$(jq -r '.padding_text//""' "$current")"; default_check="$(jq -r '.idle_session_check_interval//"30s"' "$current")"
    default_timeout="$(jq -r '.idle_session_timeout//"30s"' "$current")"; default_min="$(jq -r '.min_idle_session//0' "$current")"
    default_cm="$(jq -r '.client_metadata//""' "$current")"
    default_alpn="$(jq -r '(.alpn//[])|join(",")' "$current")"
    default_min_tls="$(jq -r '.min_tls_version//"1.2"' "$current")"
    default_max_tls="$(jq -r '.max_tls_version//"1.3"' "$current")"
  ...
  if [[ "$ANYTLS_SECURITY" == tls ]]; then
    ...
    if [[ "$ANYTLS_CERT_MODE" == selfsigned ]]; then ANYTLS_INSECURE=true
      ...
      choice=$(whiptail --title "TLS verification" --default-item "$([[ "$default_insecure" == true ]] && echo insecure || echo verify)" \
        --menu "Client verification:" 16 76 2 verify "Verify certificate" insecure "Allow insecure" 2>&1 >/dev/tty) || return 1
      [[ "$choice" == insecure ]] && ANYTLS_INSECURE=true
    fi
  fi
  ANYTLS_PADDING_MODE=$(whiptail --title "AnyTLS padding" --default-item "$default_pm" --menu "Server padding scheme:" 16 82 2 \
    default "Official default scheme" custom "Custom scheme" 2>&1 >/dev/tty) || return 1
  ANYTLS_PADDING_TEXT=""
  if [[ "$ANYTLS_PADDING_MODE" == custom ]]; then
    ANYTLS_PADDING_TEXT=$(whiptail --inputbox "Rules separated with ';' (stop=8;0=30-30;1=100-400):" 12 88 "$default_pt" 2>&1 >/dev/tty) || return 1
    anytls_padding_json custom "$ANYTLS_PADDING_TEXT" >/dev/null || { tui_error "Invalid padding scheme."; return 1; }
  fi
  ANYTLS_IDLE_CHECK=$(whiptail --inputbox "Client idle-session check interval:" 10 70 "$default_check" 2>&1 >/dev/tty) || return 1
  anytls_valid_duration "$ANYTLS_IDLE_CHECK" || { tui_error "Use a duration such as 30s or 2m."; return 1; }
...
anytls_generate_keys() {
  local output
  output="$("$ANYTLS_BINARY" generate reality-keypair 2>&1)" || { tui_error "Reality key generation failed."; return 1; }
  ANYTLS_PRIVATE="$(sed -n 's/^PrivateKey:[[:space:]]*//p' <<<"$output" | head -n1)"
  ANYTLS_PUBLIC="$(sed -n 's/^PublicKey:[[:space:]]*//p' <<<"$output" | head -n1)"
  ANYTLS_SID="$(openssl rand -hex 8)"
  [[ -n "$ANYTLS_PRIVATE" && -n "$ANYTLS_PUBLIC" && "$ANYTLS_SID" =~ ^[0-9a-f]{16}$ ]]
}
anytls_write_meta() {
...
anytls_write_service() {
  local id="$1" dir="$(anytls_dir "$1")"
  cat >"/etc/systemd/system/$(anytls_unit "$id")" <<EOF_ANYTLS_SERVICE
[Unit]
Description=GECKO AnyTLS ($id)
After=network-online.target
Wants=network-online.target
[Service]
Type=simple
ExecStart=$ANYTLS_BINARY run -c $dir/config.json
Restart=on-failure
RestartSec=3
LimitNOFILE=1048576
NoNewPrivileges=true
...
anytls_create() {
  ...
  users="$(jq -n --arg n "$name" --arg p "$password" '[{name:$n,password:$p}]')"
...
anytls_modify() {
  local id="$1" dir="$(anytls_dir "$1")" meta users padding private public sid backup
  meta="$dir/meta.json"; users="$(jq -c .users "$meta")"; private="$(jq -r '.reality_private_key//""' "$meta")"
  public="$(jq -r '.reality_public_key//""' "$meta")"; sid="$(jq -r '.reality_short_id//""' "$meta")"
  anytls_collect "$meta" || return 1
...
anytls_replace_meta() {
  ...
  backup="$(mktemp /tmp/anytls-meta-old.XXXXXX)" || return 1
  cp -a "$meta" "$backup" || { rm -f "$backup"; return 1; }
  if ! install -m 0600 "$candidate" "$meta" || ! anytls_commit "$id"; then
    cp -a "$backup" "$meta"
    rm -f "$backup"
    return 1
  fi
...
anytls_add_user() {
  local id="$1" meta="$(anytls_dir "$1")/meta.json" name password tmp rc
  name=$(whiptail --inputbox "New user name:" 10 60 "" 2>&1 >/dev/tty) || return 1
  anytls_valid_name "$name" || { tui_error "Invalid user name."; return 1; }
  jq -e --arg n "$name" 'any(.users[];.name==$n)' "$meta" >/dev/null && { tui_error "User exists."; return 1; }
  password=$(whiptail --inputbox "Password (blank = secure random):" 10 72 "" 2>&1 >/dev/tty) || return 1
  [[ -n "$password" ]] || password="$(openssl rand -base64 24 | tr -d '\n')"
  anytls_valid_password "$password" || { tui_error "Password is empty, too long or contains control characters."; return 1; }
...
anytls_remove_user() {
  local id="$1" meta="$(anytls_dir "$1")/meta.json" name tmp rc
  [[ "$(jq '.users|length' "$meta")" -gt 1 ]] || { tui_error "At least one user must remain."; return 1; }
  local -a rows=(); while read -r name; do rows+=("$name" ""); done < <(jq -r '.users[].name' "$meta")
  name=$(whiptail --title "Remove user" --menu "Select:" 20 68 10 "${rows[@]}" 2>&1 >/dev/tty) || return 1
  tmp="$(mktemp /tmp/anytls-meta.XXXXXX)" || return 1
  jq --arg n "$name" '.users|=map(select(.name!=$n))' "$meta" >"$tmp" &&
    anytls_replace_meta "$id" "$tmp"; rc=$?; rm -f "$tmp"; return "$rc"
}
anytls_client_json() {
